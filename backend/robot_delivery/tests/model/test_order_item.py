...
def test_order_item_creation(order_items):
    ...
    assert order_items[0].price == 50.00

@pytest.mark.django_db
def test_order_item_creation_no_quantity(create_orders, create_products):
    with pytest.raises(IntegrityError):
        order_item = OrderItem.objects.create(order=create_orders[0], product=create_products[0], quantity=None, price=50.00)
        
@pytest.mark.django_db
def test_order_item_creation_no_price(create_orders, create_products):
    with pytest.raises(IntegrityError):
        order_item = OrderItem.objects.create(order=create_orders[0], product=create_products[0], quantity=2, price=None)   